...
export default {
    ...
    methods: {
        mounted() {
            ...
            let sw = true;
            // 此处使用node做了代理  
             var argumen = JSON.parse(localStorage.getItem('order'));
            service.get(api.contract + 'api/v1/group/youLike?goods_name=' + argumen.name + '&goods_id=' + argumen.id).then(res => {
